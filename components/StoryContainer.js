...
const StoryContainer = (props) => {
  ...
  const story = status.length ? status[currentIndex] : {};
  ...
  const nextStory = () => {
    if (status.length - 1 > currentIndex) {
  ...
  const prevStory = () => {
    if (currentIndex > 0 && status.length) {
      setCurrentIndex(currentIndex - 1);
      setLoaded(false);
      setDuration(3);
    } else {
    ...
    >
      ...
      >
        <View style={styles.container}>
          ...
          <UserView name={user.userName} profile={user.profile.profilePic} onClosePress={props.onClose} />
          ...
          <ProgressArray
            ...
            currentStory={status[currentIndex]}
            length={status.map((_, i) => i)}
